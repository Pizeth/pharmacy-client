...
function julianDayToGregorian(jdn: number): {
  year: number;
  month: number;
  day: number;
} {
...
function validateGregorianDate(
  ...
  if (typeof minute !== "number" || isNaN(minute)) {
    throw new Error(
      `Invalid minute: ${minute}. Minute must be a valid number.`,
    );
  }
  if (typeof second !== "number" || isNaN(second)) {
    throw new Error(
      `Invalid second: ${second}. Second must be a valid number.`,
    );
  }
...
function validateKhmerDate(
  ...
  if (typeof moonPhase !== "number" || isNaN(moonPhase)) {
    throw new Error(
      `Invalid moonPhase: ${moonPhase}. moonPhase must be a valid number.`,
    );
  }
  if (typeof monthIndex !== "number" || isNaN(monthIndex)) {
    throw new Error(
      `Invalid monthIndex: ${monthIndex}. monthIndex must be a valid number.`,
    );
  }
  if (typeof beYear !== "number" || isNaN(beYear)) {
    throw new Error(
      `Invalid beYear: ${beYear}. beYear must be a valid number.`,
    );
  }
  ...
  const monthIndexNum =
    typeof monthIndex === "number" ? monthIndex : monthIndex;
...
function isKhmerLeapMonth(beYear: number): boolean {
  ...
  return (
    (bodithey === 24 && boditheyNextYear === 6) ||
    bodithey >= 25 ||
    bodithey < 6
  );
}
...
function getNumberOfDaysInKhmerMonth(
  monthIndex: MonthIndex | number,
  beYear: number,
): number {
  ...
  if (idx === MonthIndex.Jesth && leapType === 2) {
    // ជេស្ឋ with leap day
    return 30;
  }

  if (idx === MonthIndex.Pathamasadh || idx === MonthIndex.Tutiyasadh) {
    // បឋមាសាឍ, ទុតិយាសាឍ
...
function nextMonthOf(
  ...
  if (idx === MonthIndex.Jesth && leapType === 1) {
    // ជេស្ឋ in leap month year
...
function getSunInfo(jsYear: number, sotin: number): SunInfo {
  ...
  // Chhaya sun
  const chhayaSunMap: { multiplicity: number; chhaya: number }[] = [
...
class KhmerDate {
  ...
  getDayNumber(): number {
    if (this.moonPhase === MoonPhase.Waxing) {
      // កើត
  ...
  addDays(count: number): KhmerDate {
    ...
    while (remaining > 0) {
      const daysInMonth = getNumberOfDaysInKhmerMonth(
        result.monthIndex,
        result.beYear,
      );
      ...
      if (remaining <= daysLeftInMonth) {
        ...
        if (result.monthIndex === MonthIndex.Pisakh) {
          // ពិសាខ
          if (
            result.moonPhase === MoonPhase.Waxing &&
            newDay.moonPhase === MoonPhase.Waning
          ) {
            newBeYear++;
          }
        }

        result = new KhmerDate(
          newDay.day,
          newDay.moonPhase,
          result.monthIndex,
          newBeYear,
        ...
        const newBeYear =
          result.monthIndex === MonthIndex.Cheit
            ? result.beYear + 1
            : result.beYear;
    ...
    while (remaining > 0) {
      ...
      if (remaining <= currentDayNum) {
        ...
        if (result.monthIndex === MonthIndex.Pisakh) {
          // ពិសាខ
          if (
            result.moonPhase === MoonPhase.Waning &&
            newDay.moonPhase === MoonPhase.Waxing
          ) {
            newBeYear--;
          }
        }
        ...
        const newBeYear =
          result.monthIndex === MonthIndex.Pisakh
            ? result.beYear - 1
            : result.beYear;
        const daysInPrevMonth = getNumberOfDaysInKhmerMonth(
          prevMonth,
          newBeYear,
        );
...
function getPisakhaBochea(year: number, isSearching: boolean = false): number {
  ...
  for (let searchMonth = 4; searchMonth <= 6; searchMonth++) {
    ...
    for (let searchDay = 1; searchDay <= daysInMonth; searchDay++) {
      ...
      const result = gregorianToKhmerInternal(
        year,
        searchMonth,
        searchDay,
        12,
      ...
      if (
        result.khmer.monthIndex === MonthIndex.Pisakh &&
        result._khmerDateObj.getDayNumber() === 15
      ) {
...
function gregorianToKhmerInternal(
  ...
  let khmerMonth: MonthIndex = 1; // បុស្ស
  ...
  if (!isSearching) {
    ...
    if (
      inputTimestamp >= newYearInfo.newYearMoment.getTime() &&
      inputTimestamp <= visakhaBocheaTimestamp
    ) {
    ...
    if (
      inputTimestamp >= newYearInfo.lerngSakMoment.getTime() &&
      inputTimestamp <= visakhaBocheaTimestamp
    ) {
      jsYear++;
    }
...
function khmerToGregorian(
  ...
  for (let year = startYear; year <= endYear; year++) {
    for (let month = 1; month <= 12; month++) {
      ...
      for (let gDay = 1; gDay <= daysInMonth; gDay++) {
        ...
        const isAroundBEYearChange =
          monthIndexNum === MonthIndex.Pisakh &&
          ((day === 15 && moonPhaseNum === MoonPhase.Waxing) ||
            (day === 1 && moonPhaseNum === MoonPhase.Waning));
        ...
        for (const hour of timesToCheck) {
          const khmerResult = gregorianToKhmerInternal(
            year,
            month,
            gDay,
            hour,
          ...
          if (
            khmerResult.khmer.beYear === beYear &&
  ...
  if (candidates.length === 0) {
    throw new Error(
      `Could not find Gregorian date for Khmer date: ${day} ${moonPhaseNum === MoonPhase.Waxing ? "កើត" : "រោច"} month ${monthIndexNum} BE ${beYear}`,
    );
  }
  ...
  if (candidates.length > 1) {
    ...
    const minDistance = Math.min(
      ...candidates.map((c) => Math.abs(c.year - approxYear)),
    );
    const closestCandidates = candidates.filter(
      (c) => Math.abs(c.year - approxYear) === minDistance,
    );
    ...
    const noonMatches = closestCandidates.filter((c) => {
      const noonCheck = gregorianToKhmerInternal(
        c.year,
        c.month,
        c.day,
        12,
...
function getNewYearFullInfo(ceYear: number): NewYearFullInfo {
  ...
  const numberNewYearDay =
    newYearInfo.newYearsDaySotins[0].angsar === 0 ? 4 : 3;
  ...
  const khEpoch = gregorianToKhmerInternal(
    ceYear,
    4,
    17,
    12,
  ...
  const diffFromEpoch =
    (khEpoch.monthIndex - 4) * 29 +
    khEpoch.getDayNumber() -
  ...
  const epochJdn = gregorianToJulianDay(
    epochLerngSakGreg.year,
    epochLerngSakGreg.month,
    epochLerngSakGreg.day,
  );
  ...
  const lerngSakMoment = new Date(
    lerngSakDate.year,
    lerngSakDate.month - 1,
    lerngSakDate.day,
    0,
    0,
    0,
...
function formatKhmer(
  khmerData: KhmerConversionResult,
  formatString?: string,
): string {
  if (!formatString) {
    // Default format
  ...
  const formatRules: Record<string, () => string | number> = {
    ...
    D: () =>
      toKhmerNumeral(
        (khmerData.khmer.day < 10 ? "0" : "") + khmerData.khmer.day,
      ),
    ...
    Ds: () =>
      toKhmerNumeral(
        (khmerData.gregorian.day < 10 ? "0" : "") + khmerData.gregorian.day,
      ),
...
function gregorianToKhmer(
  ...
  return gregorianToKhmerInternal(
    year,
    month,
    day,
    hour,
